...
- **One Document per cutscene.** A Document only syncs replicas for clips inside its own folder. Export once wrote ids for "All cutscenes" through the open cutscene's Document, so the other cutscenes kept every replica (about 200k poses in ReplicatedStorage) although they were fully exported. Write each target through a Document for its own cutscene, and tidy every cutscene in scope afterwards.
