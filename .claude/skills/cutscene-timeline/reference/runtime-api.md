...
- **Anything computed from keyframes is wrong on clients once clips are uploaded.** `ctx:PoseAt`, `ctx:ProbeJointCFrame` and anything built on them sample keyframes the client no longer has, so they return the rest pose. A flask left "where the hand was at the end of the clip" floated sideways in mid-air. In game, capture the live transform instead (the last frame before the event), and only probe in the preview.
- **Prewarm or drift.** An animation loaded on its clip's Enter, or a sound that loads after its start, begins late. A drift threshold (0.15 to 0.3 s) then leaves it permanently behind. Load every track and preload sounds when the timeline is built. On the first frame an asset reports a length, jump it to the expected position once, then keep the threshold for real drift.
- **First person hides the local character.** The PlayerModule's TransparencyController sets `LocalTransparencyModifier` on the local character every frame at the Camera render priority. A cutscene that uses the real character must write its own visibility after that (Camera + 1), and treat the character's original transparency as 0.
